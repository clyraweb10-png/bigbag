...
export default async function QualificationPage() {
  ...
  for (const project of QUALIFICATION_PROJECTS) {
    ...
    counts[countKey] += 1;
    if (String(evidence?.firstAttemptResult) === "FAIL" && status === "PASS") repaired += 1;
    if (status === "RUNNING") running += 1;
    if (status === "RETESTING") retesting += 1;
  }
  ...
  return (
    <main className="min-h-screen bg-[#111113] px-4 py-8 text-zinc-100 sm:px-7 lg:px-10">
      <div className="mx-auto max-w-[1500px] space-y-7">
        ...
        <section aria-label="Campaign totals" className="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-9">
          {[["TOTAL", 100], ["RUNNING", running], ["PASS", counts.PASS], ["FAIL", counts.FAIL], ["BLOCKED", counts.BLOCKED], ["PARTIAL", counts.PARTIAL], ["REPAIRED", repaired], ["RETESTING", retesting], ["NOT RUN", counts.NOT_RUN]].map(([label, value]) => (
              ...
              <details key={project.projectNumber} className="group rounded-2xl border border-white/10 bg-[#19191c] p-4 open:border-violet-400/30">
                <summary className="cursor-pointer list-none">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-1.5 text-[11px] text-zinc-400">
                    {[["Build", "build"], ["Runtime", "runtime"], ["Preview", "preview"], ["Auth", "authentication"], ["Database", "database"], ["CRUD", "crud"], ["Connector", "connector"], ["Responsive", "responsive"], ["Accessibility", "accessibility"], ["Security", "security"], ["Edit", "edit"], ["CodeRabbit", "codeRabbit"]].map(([label, key]) => <div key={key} className="flex justify-between gap-2"><span>{label}</span><span className="font-mono">{valueAt(evidence, key)}</span></div>)}
                  </div>
                </summary>
        ...
        <section className="rounded-2xl border border-white/10 bg-[#19191c] p-5">
          <div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold">Connector qualification</h2><p className="mt-1 text-sm text-zinc-400">Provider operations are separate from catalogue availability; source inspection alone never produces PASS.</p></div><Pill value="FAIL" /></div>
          <div className="mt-4 overflow-x-auto" tabIndex={0} role="region" aria-label="Connector qualification results">
            <table className="w-full min-w-[1500px] text-left text-xs">
              <thead className="text-zinc-300"><tr>{["Connector", "Status", "Auth", "Read", "Write", "Update", "Delete", "Error handling", "Rate limit", "Reconnect", "Security", "Generated app", "Last test", "Failures / fixes", "CodeRabbit"].map((heading) => <th key={heading} className="border-b border-white/10 px-3 py-2 font-medium">{heading}</th>)}</tr></thead>
              <tbody className="text-zinc-300">
                <tr>{["Supabase Auth + PostgreSQL", "PARTIAL", "PASS", "PASS", "PASS", "PASS", "PASS", "PARTIAL", "NOT_RUN", "PARTIAL", "PARTIAL", "PARTIAL", "Current campaign", "Real DB suite and one generated-app browser flow; campaign-wide browser evidence pending", "FINAL REVIEW PENDING"].map((value, index) => <td key={index} className="border-b border-white/5 px-3 py-3">{index === 1 ? <Pill value={value} /> : value}</td>)}</tr>
                <tr>{["Firecrawl visual reference", "NOT_RUN", "NOT_RUN", "NOT_RUN", "NOT APPLICABLE", "NOT APPLICABLE", "NOT APPLICABLE", "NOT_RUN", "NOT_RUN", "NOT_RUN", "PARTIAL", "NOT_RUN", "Designer batches pending", "Implementation tests exist; real designer campaign operations pending", "FINAL REVIEW PENDING"].map((value, index) => <td key={index} className="border-b border-white/5 px-3 py-3">{index === 1 ? <Pill value={value} /> : value}</td>)}</tr>
                <tr>{["Pexels imagery", "BLOCKED", "BLOCKED", "BLOCKED", "NOT APPLICABLE", "NOT APPLICABLE", "NOT APPLICABLE", "PARTIAL", "NOT_RUN", "NOT_RUN", "PARTIAL", "NOT_RUN", "Credential unavailable", "No configured provider credential; unit behavior does not count as provider PASS", "FINAL REVIEW PENDING"].map((value, index) => <td key={index} className="border-b border-white/5 px-3 py-3">{index === 1 ? <Pill value={value} /> : value}</td>)}</tr>
                <tr>{["GitHub sync + Figma status", "FAIL", "FAIL", "FAIL", "FAIL", "FAIL", "FAIL", "FAIL", "FAIL", "FAIL", "PARTIAL", "FAIL", "Current campaign", "Local status-only stubs exist; connection operations are not mapped", "FINAL REVIEW PENDING"].map((value, index) => <td key={index} className="border-b border-white/5 px-3 py-3">{index === 1 ? <Pill value={value} /> : value}</td>)}</tr>
                <tr>{["28 remaining catalogue adapters", "FAIL", "FAIL", "FAIL", "FAIL", "FAIL", "FAIL", "FAIL", "FAIL", "FAIL", "PARTIAL", "FAIL", "Current campaign", "UI is fail-closed; server adapters do not exist", "FINAL REVIEW PENDING"].map((value, index) => <td key={index} className="px-3 py-3">{index === 1 ? <Pill value={value} /> : value}</td>)}</tr>
              </tbody>
            </table>
          </div>
        </section>
