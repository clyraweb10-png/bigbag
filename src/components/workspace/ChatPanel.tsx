...
export function ChatPanel({
  ...
  useEffect(() => { if (textareaRef.current) { textareaRef.current.style.height = "auto"; textareaRef.current.style.height = Math.min(textareaRef.current.scrollHeight, 128) + "px"; } }, [prompt]);
      ...
      <div className="shrink-0 px-3 pb-3 pt-2">
        <div className="rounded-2xl border border-border overflow-hidden transition-all focus-within:ring-2 focus-within:ring-ring focus-within:border-primary/50 shadow-xs bg-card dark:bg-[#444444]">
          <textarea data-chat-input ref={textareaRef} value={prompt} onChange={(e) => setPrompt(e.target.value)} onKeyDown={handleKeyDown} onPaste={handlePaste}
            ...
            className="w-full bg-transparent border-0 resize-none text-base outline-none placeholder:text-muted-foreground min-h-[48px] max-h-[128px] px-4 pt-3.5 pb-1 leading-relaxed text-foreground"
            ...
            */}
            ...
            {isBuilding ? (
              ...
              <button onClick={handleSend} disabled={(!prompt.trim() && attachedFiles.length === 0) || sending}
                ...
                className="w-8 h-8 rounded-full colourless-glass disabled:opacity-40 flex items-center justify-center transition-all cursor-pointer">
