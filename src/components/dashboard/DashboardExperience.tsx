...
export function DashboardContent() {
  ...
  const openBuildModal = (promptOverride?: string) => {
    const buildPrompt =
      ...
      landingMessages
        ...
        .filter(Boolean)
        .join(" ");
    if (!buildPrompt && attachedFiles.length === 0 && !figmaToken) return;
    setBuildName("");
    setBuildError(null);
  ...
  const submitLandingMessage = async () => {
    ...
    if ((!message && attachedFiles.length === 0 && !figmaToken) || plannerRunning || buildCreating) return;

    if (attachedFiles.length > 0 || figmaToken) {
      if (message) setApprovedPrompt(message);
                ...
                <div className={`${landingMessages.length > 0 ? "mt-3 shrink-0" : ""} rounded-2xl bg-card dark:bg-[#444444] border border-border/80 dark:border-0 overflow-hidden focus-within:ring-2 focus-within:ring-ring/25 transition-all`}>
                  ...
                  <div className="flex items-center justify-between bg-transparent px-3 py-3 sm:px-4">
                    ...
                    <button
                      ...
                      disabled={(!firstPrompt.trim() && attachedFiles.length === 0 && !figmaToken) || plannerRunning || buildCreating}
                      aria-label="Send"
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
              ...
              <div className="flex-1 flex flex-wrap items-center gap-2 sm:justify-end">
                ...
                <div className="relative flex-1 sm:flex-none sm:w-56 min-w-[160px]">
                  ...
                  />
                  {search && (
                    <button aria-label="Clear project search" onClick={() => { setSearch(""); setPage(1); }} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                      <X className="w-3 h-3" />
                    </button>
                  )}
                </div>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {pageItems.map((p) => {
                  ...
                  const relTime = diffMins < 60
                    ? `${displayMins} min${displayMins !== 1 ? "s" : ""} ago`
                    : diffMins < 1440
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <button
                                  ...
                                  title="Options"
                                  className="opacity-0 group-hover:opacity-100 focus-visible:opacity-100 data-[state=open]:opacity-100 w-6 h-6 rounded-md flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-accent transition-all shrink-0"
                                >
                      ...
                      {pageItems.map((p) => (
                        ...
                        >
                          ...
                          <td className="px-2 py-2">
                            <Link href={`/project/${p.projectId}`} onClick={(event) => event.stopPropagation()} className="text-sm font-medium text-foreground group-hover:text-primary transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary">{p.projectId}</Link>
                          </td>
