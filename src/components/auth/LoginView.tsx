...
export function LoginView({ onFlipBack, className = "" }: LoginViewProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // If destination is dashboard and we are on /login or /dashboard, let the 3D flip transition handle it cleanly
      if (
        ...
        if (window.location.pathname === "/login") {
          try {
            window.history.pushState({ flip: false }, "", "/dashboard");
          } catch {}
        }
        return;
      }
  ...
  /* While loading or after auth, show preloader screen if not handled by 3D flip */
  if (status === "authenticated" && user && !onFlipBack) {
    return <SkeletonDashboard />;
  }

  return (
    <main
      className={`relative min-h-[100dvh] w-full overflow-hidden bg-white dark:bg-[#1d1d1c] text-zinc-900 dark:text-white flex flex-col justify-between select-none transition-colors duration-200 ${className}`}
    >
        ...
        <div className="flex items-center gap-3">
          {onFlipBack ? (
            <button
              type="button"
              onClick={onFlipBack}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-zinc-200 dark:border-white/15 bg-white/80 dark:bg-white/5 hover:bg-zinc-100 dark:hover:bg-white/10 text-xs font-semibold text-zinc-700 dark:text-zinc-200 shadow-xs transition-all cursor-pointer"
